...
case "${UGV_PROFILE}" in
  ...
  round2_row_transition_test)
    ...
    profile_default SWEEP_TRANSITION_MIN_EMITTED_V_MPS "0.12"
    profile_default SWEEP_TRANSITION_MIN_EMITTED_OMEGA_RPS "0.20"
    profile_default SWEEP_TRANSITION_MAX_EMITTED_OMEGA_RPS "0.40"
    profile_default SWEEP_TRANSITION_INNER_WHEEL_MIN_MPS "0.04"
    profile_default SWEEP_TRANSITION_TIMEOUT_S "20.0"
    profile_default SWEEP_TRANSITION_DIVERGENCE_ACTION "stop"
    profile_default SWEEP_CELL_SIZE_M "0.75"
...
SWEEP_TURN_STYLE="${SWEEP_TURN_STYLE:-segmented_90}"
...
SWEEP_TURN_90_YAW_TOLERANCE_DEG="${SWEEP_TURN_90_YAW_TOLERANCE_DEG:-7.0}"
SWEEP_TURN_STRONG_ERROR_DEG="${SWEEP_TURN_STRONG_ERROR_DEG:-30.0}"
SWEEP_TURN_CAPTURE_ERROR_DEG="${SWEEP_TURN_CAPTURE_ERROR_DEG:-12.0}"
SWEEP_TURN_SETTLE_ERROR_DEG="${SWEEP_TURN_SETTLE_ERROR_DEG:-7.0}"
SWEEP_TURN_SETTLE_FRAMES="${SWEEP_TURN_SETTLE_FRAMES:-3}"
SWEEP_PIVOT_MIN_EMITTED_OMEGA_RPS="${SWEEP_PIVOT_MIN_EMITTED_OMEGA_RPS:-0.25}"
SWEEP_PIVOT_MAX_EMITTED_OMEGA_RPS="${SWEEP_PIVOT_MAX_EMITTED_OMEGA_RPS:-0.45}"
SWEEP_PIVOT_WHEEL_MIN_MPS="${SWEEP_PIVOT_WHEEL_MIN_MPS:-0.08}"
SWEEP_PIVOT_SETTLE_FRAMES="${SWEEP_PIVOT_SETTLE_FRAMES:-${SWEEP_TURN_SETTLE_FRAMES}}"
SWEEP_CROSS_LANE_V_MPS="${SWEEP_CROSS_LANE_V_MPS:-0.12}"
SWEEP_CROSS_LANE_HEADING_KP="${SWEEP_CROSS_LANE_HEADING_KP:-1.10}"
SWEEP_LANE_CAPTURE_TOLERANCE_M="${SWEEP_LANE_CAPTURE_TOLERANCE_M:-0.20}"
SWEEP_YAW_CAPTURE_TOLERANCE_DEG="${SWEEP_YAW_CAPTURE_TOLERANCE_DEG:-8.0}"
SWEEP_TRANSITION_MIN_EMITTED_V_MPS="${SWEEP_TRANSITION_MIN_EMITTED_V_MPS:-0.12}"
SWEEP_TRANSITION_MIN_EMITTED_OMEGA_RPS="${SWEEP_TRANSITION_MIN_EMITTED_OMEGA_RPS:-0.20}"
SWEEP_TRANSITION_MAX_EMITTED_OMEGA_RPS="${SWEEP_TRANSITION_MAX_EMITTED_OMEGA_RPS:-0.40}"
SWEEP_TRANSITION_INNER_WHEEL_MIN_MPS="${SWEEP_TRANSITION_INNER_WHEEL_MIN_MPS:-0.04}"
SWEEP_TRANSITION_TIMEOUT_S="${SWEEP_TRANSITION_TIMEOUT_S:-20.0}"
SWEEP_TRANSITION_DIVERGENCE_ACTION="${SWEEP_TRANSITION_DIVERGENCE_ACTION:-stop}"
...
MOTOR_CONTROL_LOCATION="${MOTOR_CONTROL_LOCATION:-ros}"
MOTOR_CONTROL_LOCATION="$(printf '%s' "${MOTOR_CONTROL_LOCATION}" | tr '[:upper:]' '[:lower:]')"
case "${MOTOR_CONTROL_LOCATION}" in
  teensy|teensy_side_pid)
...
if [[ "${MOTOR_CONTROL_LOCATION}" == "teensy_pid" ]]; then
...
MOTOR_TEENSY_LEFT_MOTOR_SIGN="${MOTOR_TEENSY_LEFT_MOTOR_SIGN:-1}"
MOTOR_TEENSY_RIGHT_MOTOR_SIGN="${MOTOR_TEENSY_RIGHT_MOTOR_SIGN:--1}"
MOTOR_TEENSY_FL_ENCODER_SIGN="${MOTOR_TEENSY_FL_ENCODER_SIGN:-1}"
MOTOR_TEENSY_FR_ENCODER_SIGN="${MOTOR_TEENSY_FR_ENCODER_SIGN:-1}"
MOTOR_TEENSY_RL_ENCODER_SIGN="${MOTOR_TEENSY_RL_ENCODER_SIGN:-1}"
MOTOR_TEENSY_RR_ENCODER_SIGN="${MOTOR_TEENSY_RR_ENCODER_SIGN:-1}"
...
if [[ "${MOTOR_CONTROL_LOCATION}" == "teensy_pid" ]]; then
  if awk -v radius="${MOTOR_WHEEL_RADIUS_M}" -v ticks="${MOTOR_TICKS_PER_REV}" 'BEGIN {
    exit !((radius > 0.087 && radius < 0.091) && (ticks > 2100 && ticks < 2200))
  }'; then
    echo "ERROR: MOTOR_CONTROL_LOCATION=teensy_pid must not mix physical wheel radius ${MOTOR_WHEEL_RADIUS_M} with legacy effective ticks ${MOTOR_TICKS_PER_REV}."
    echo "Use MOTOR_WHEEL_RADIUS_M=0.0889 with MOTOR_TICKS_PER_REV=3200, or explicitly use the full legacy effective pair."
    exit 1
  fi
fi
...
echo "UGV robot track width: ROBOT_TRACK_WIDTH_M=${ROBOT_TRACK_WIDTH_M}"
echo "UGV motor velocity odometry: MOTOR_WHEEL_RADIUS_M=${MOTOR_WHEEL_RADIUS_M}, MOTOR_TICKS_PER_REV=${MOTOR_TICKS_PER_REV}, MOTOR_VELOCITY_CONTROL_ENABLED=${MOTOR_VELOCITY_CONTROL_ENABLED}, MOTOR_CONTROL_LOCATION=${MOTOR_CONTROL_LOCATION}"
echo "UGV Teensy PID signs: motor_left/right=${MOTOR_TEENSY_LEFT_MOTOR_SIGN}/${MOTOR_TEENSY_RIGHT_MOTOR_SIGN}, encoder_fl/fr/rl/rr=${MOTOR_TEENSY_FL_ENCODER_SIGN}/${MOTOR_TEENSY_FR_ENCODER_SIGN}/${MOTOR_TEENSY_RL_ENCODER_SIGN}/${MOTOR_TEENSY_RR_ENCODER_SIGN}"
...
echo "UGV sweep field: width=${SWEEP_FIELD_WIDTH_M} height=${SWEEP_FIELD_HEIGHT_M} row_length=${SWEEP_ROW_LENGTH_M} headland=${SWEEP_HEADLAND_MARGIN_M} boundary=${SWEEP_BOUNDARY_MARGIN_M} turn_style=${SWEEP_TURN_STYLE} turn_radius=${SWEEP_TURN_RADIUS_M} row_transition=${SWEEP_ROW_TRANSITION_ENABLED} max_rows=${SWEEP_MAX_ROWS}"
echo "UGV row transition: turn90_tol=${SWEEP_TURN_90_YAW_TOLERANCE_DEG}deg strong=${SWEEP_TURN_STRONG_ERROR_DEG}deg capture=${SWEEP_TURN_CAPTURE_ERROR_DEG}deg settle=${SWEEP_TURN_SETTLE_ERROR_DEG}deg settle_frames=${SWEEP_TURN_SETTLE_FRAMES} pivot_min_omega=${SWEEP_PIVOT_MIN_EMITTED_OMEGA_RPS} pivot_max_omega=${SWEEP_PIVOT_MAX_EMITTED_OMEGA_RPS} pivot_wheel_min=${SWEEP_PIVOT_WHEEL_MIN_MPS} pivot_settle_frames=${SWEEP_PIVOT_SETTLE_FRAMES} cross_v=${SWEEP_CROSS_LANE_V_MPS} cross_heading_kp=${SWEEP_CROSS_LANE_HEADING_KP} lane_tol=${SWEEP_LANE_CAPTURE_TOLERANCE_M}m yaw_tol=${SWEEP_YAW_CAPTURE_TOLERANCE_DEG}deg min_v=${SWEEP_TRANSITION_MIN_EMITTED_V_MPS} min_omega=${SWEEP_TRANSITION_MIN_EMITTED_OMEGA_RPS} max_omega=${SWEEP_TRANSITION_MAX_EMITTED_OMEGA_RPS} inner_min=${SWEEP_TRANSITION_INNER_WHEEL_MIN_MPS} timeout=${SWEEP_TRANSITION_TIMEOUT_S}s action=${SWEEP_TRANSITION_DIVERGENCE_ACTION}"
...
ros2 launch "${WORKSPACE_DIR}/src/ugv_sensor_sync/launch/competition_bringup.launch.py" \
  ...
  motor_teensy_left_motor_sign:="${MOTOR_TEENSY_LEFT_MOTOR_SIGN}" \
  motor_teensy_right_motor_sign:="${MOTOR_TEENSY_RIGHT_MOTOR_SIGN}" \
  motor_teensy_fl_encoder_sign:="${MOTOR_TEENSY_FL_ENCODER_SIGN}" \
  motor_teensy_fr_encoder_sign:="${MOTOR_TEENSY_FR_ENCODER_SIGN}" \
  motor_teensy_rl_encoder_sign:="${MOTOR_TEENSY_RL_ENCODER_SIGN}" \
  motor_teensy_rr_encoder_sign:="${MOTOR_TEENSY_RR_ENCODER_SIGN}" \
