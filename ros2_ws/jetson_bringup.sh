...
case "${UGV_PROFILE}" in
  ...
  round2_competition_tuned)
    profile_default ROUND_MODE "round2"
    profile_default START_ZED "true"
    profile_default START_YOLO_OBSTACLES "false"
    profile_default START_MARKER_VISION "true"
    profile_default START_DEBUG_DASHBOARD "true"
    profile_default NAV_ACTIVE_SCAN_ENABLED "false"
    profile_default SWEEP_CELL_SIZE_M "0.75"
    profile_default SWEEP_LANE_SPACING_M "0.75"
    profile_default SWEEP_COVERAGE_RADIUS_M "0.55"
    profile_default SWEEP_COVERAGE_THRESHOLD "0.85"
    profile_default SWEEP_GOAL_TIMEOUT_S "12.0"
    profile_default SWEEP_FAIL_LIMIT "3"
    profile_default ROBOT_TICKS_PER_REV "2151"
    profile_default ROBOT_WHEEL_RADIUS_M "0.06"
...
SWEEP_LANE_TOLERANCE_M="${SWEEP_LANE_TOLERANCE_M:-0.30}"
...
NAV_COMPETITION_CLOSED_LOOP_ENABLED="${NAV_COMPETITION_CLOSED_LOOP_ENABLED:-true}"
NAV_HEADING_HOLD_ENABLED="${NAV_HEADING_HOLD_ENABLED:-true}"
NAV_LANE_FOLLOW_ENABLED="${NAV_LANE_FOLLOW_ENABLED:-true}"
NAV_HEADING_HOLD_KP="${NAV_HEADING_HOLD_KP:-1.10}"
NAV_HEADING_HOLD_KD="${NAV_HEADING_HOLD_KD:-0.05}"
NAV_HEADING_HOLD_DEADBAND_DEG="${NAV_HEADING_HOLD_DEADBAND_DEG:-3.0}"
NAV_HEADING_HOLD_MAX_OMEGA_RPS="${NAV_HEADING_HOLD_MAX_OMEGA_RPS:-0.55}"
NAV_LANE_FOLLOW_KP_HEADING="${NAV_LANE_FOLLOW_KP_HEADING:-1.10}"
NAV_LANE_FOLLOW_KP_OMEGA="${NAV_LANE_FOLLOW_KP_OMEGA:-1.00}"
...
NAV_OMEGA_COMMAND_SIGN="${NAV_OMEGA_COMMAND_SIGN:-1.0}"
NAV_HEADING_ERROR_SIGN="${NAV_HEADING_ERROR_SIGN:-1.0}"
NAV_LANE_ERROR_SIGN="${NAV_LANE_ERROR_SIGN:-1.0}"
NAV_LANE_CORRECTION_SIGN="${NAV_LANE_CORRECTION_SIGN:-1.0}"
NAV_CLOSED_LOOP_HEALTH_ENABLED="${NAV_CLOSED_LOOP_HEALTH_ENABLED:-true}"
NAV_CLOSED_LOOP_DIVERGENCE_WINDOW="${NAV_CLOSED_LOOP_DIVERGENCE_WINDOW:-5}"
NAV_CLOSED_LOOP_DIVERGENCE_MIN_ERROR_M="${NAV_CLOSED_LOOP_DIVERGENCE_MIN_ERROR_M:-0.08}"
NAV_CLOSED_LOOP_DIVERGENCE_MAX_GROWTH_M="${NAV_CLOSED_LOOP_DIVERGENCE_MAX_GROWTH_M:-0.05}"
NAV_CLOSED_LOOP_DIVERGENCE_ACTION="${NAV_CLOSED_LOOP_DIVERGENCE_ACTION:-slow_then_stop}"
...
MOTOR_VELOCITY_CONTROL_ENABLED_WAS_SET="${MOTOR_VELOCITY_CONTROL_ENABLED+x}"
...
MOTOR_WHEEL_RADIUS_M_WAS_SET="${MOTOR_WHEEL_RADIUS_M+x}"
MOTOR_TICKS_PER_REV_WAS_SET="${MOTOR_TICKS_PER_REV+x}"
...
if [[ -z "${MOTOR_VELOCITY_CONTROL_ENABLED_WAS_SET}" && "${ROUND_MODE}" =~ ^round[123]$ ]]; then
  MOTOR_VELOCITY_CONTROL_ENABLED=true
fi

if [[ "${MOTOR_VELOCITY_CONTROL_ENABLED}" == "true" ]]; then
  MOTOR_TICKS_DELTA_PCT="$(
    awk -v motor="${MOTOR_TICKS_PER_REV}" -v robot="${ROBOT_TICKS_PER_REV}" 'BEGIN {
      if (robot <= 0) {
        print 0.0
      } else {
        delta = motor - robot
        ...
        printf "%.3f", 100.0 * delta / robot
      }
    }'
  )"
  if awk -v pct="${MOTOR_TICKS_DELTA_PCT}" 'BEGIN { exit !(pct > 5.0) }'; then
    echo "WARNING: MOTOR_VELOCITY_CONTROL_ENABLED=true but MOTOR_TICKS_PER_REV=${MOTOR_TICKS_PER_REV} differs from ROBOT_TICKS_PER_REV=${ROBOT_TICKS_PER_REV} by ${MOTOR_TICKS_DELTA_PCT}%."
  fi
fi
...
echo "UGV profile resolved: START_ZED=${START_ZED}, START_MARKER_VISION=${START_MARKER_VISION}, NAV_ACTIVE_SCAN_ENABLED=${NAV_ACTIVE_SCAN_ENABLED}, SWEEP_COVERAGE_THRESHOLD=${SWEEP_COVERAGE_THRESHOLD}, SWEEP_GOAL_TIMEOUT_S=${SWEEP_GOAL_TIMEOUT_S}, MOTOR_PORT=${MOTOR_PORT}, LIDAR_PORT=${LIDAR_PORT}"
...
ros2 launch "${WORKSPACE_DIR}/src/ugv_sensor_sync/launch/competition_bringup.launch.py" \
  ...
  competition_closed_loop_enabled:="${NAV_COMPETITION_CLOSED_LOOP_ENABLED}" \
  heading_hold_enabled:="${NAV_HEADING_HOLD_ENABLED}" \
  lane_follow_enabled:="${NAV_LANE_FOLLOW_ENABLED}" \
  heading_hold_kp:="${NAV_HEADING_HOLD_KP}" \
  heading_hold_kd:="${NAV_HEADING_HOLD_KD}" \
  heading_hold_deadband_deg:="${NAV_HEADING_HOLD_DEADBAND_DEG}" \
  heading_hold_max_omega_rps:="${NAV_HEADING_HOLD_MAX_OMEGA_RPS}" \
  lane_follow_kp_heading:="${NAV_LANE_FOLLOW_KP_HEADING}" \
  lane_follow_kp_omega:="${NAV_LANE_FOLLOW_KP_OMEGA}" \
  ...
  omega_command_sign:="${NAV_OMEGA_COMMAND_SIGN}" \
  heading_error_sign:="${NAV_HEADING_ERROR_SIGN}" \
  lane_error_sign:="${NAV_LANE_ERROR_SIGN}" \
  lane_correction_sign:="${NAV_LANE_CORRECTION_SIGN}" \
  closed_loop_health_enabled:="${NAV_CLOSED_LOOP_HEALTH_ENABLED}" \
  closed_loop_divergence_window:="${NAV_CLOSED_LOOP_DIVERGENCE_WINDOW}" \
  closed_loop_divergence_min_error_m:="${NAV_CLOSED_LOOP_DIVERGENCE_MIN_ERROR_M}" \
  closed_loop_divergence_max_growth_m:="${NAV_CLOSED_LOOP_DIVERGENCE_MAX_GROWTH_M}" \
  closed_loop_divergence_action:="${NAV_CLOSED_LOOP_DIVERGENCE_ACTION}" \
