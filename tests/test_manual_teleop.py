...
def test_wasd_velocity_mapping_uses_tank_drive_sign_convention():
    config = TeleopConfig(forward_mps=0.15, reverse_mps=0.1, turn_radps=0.35)
    ...
    assert velocity_for_key("a", config)[:2] == pytest.approx((0.0, 0.35))
    assert velocity_for_key("a", config)[2] == "manual_pivot_left"
    assert velocity_for_key("d", config)[:2] == pytest.approx((0.0, -0.35))
...
def test_normalized_config_clamps_publish_rate_and_positive_speeds():
    ...
    assert config.turn_radps == pytest.approx(0.25)
...
def test_speed_and_turn_adjustment_keys_update_config():
    ...
    assert turn_up.config.turn_radps == pytest.approx(0.38)
    turn_down = apply_key_action("[", turn_up.config)
    assert turn_down.config.turn_radps == pytest.approx(0.33)
