#!/usr/bin/env python3
"""Receive UAV ESP target packets and publish /ugv/uav_target.

This file keeps the historical ``uwb_node`` entrypoint, but the node now acts
as the UGV-side UAV target serial adapter. The UGV ESP should receive the
14-byte UAV ESP-NOW packet and forward those bytes to this process.
"""
...
try:
    ...
    for parent in Path(__file__).resolve().parents:
        candidate = parent.parent / "ugv_nav"
        if (candidate / "ugv_nav_core" / "uav_target_input.py").exists():
...
class UavTargetReceiverNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("serial_protocol", "binary14")
        ...
        if self.serial_protocol not in VALID_SERIAL_PROTOCOLS:
            self.get_logger().error(f"Unsupported serial_protocol={self.serial_protocol!r}; using binary14.")
            self.serial_protocol = "binary14"
        ...
        self.get_logger().warn(
            "UAV ESP target receiver started "
