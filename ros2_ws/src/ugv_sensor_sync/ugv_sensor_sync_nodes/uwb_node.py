...
class UwbNode(Node):
    def __init__(self):
        ...
        self.drift_a = 1.0
        self.drift_b = 0.0
        self.ser = None  # Open lazily because the ESP32 may not be plugged in yet.

        self._try_open_serial()

        # Main read loop, also handles reconnect if the cable is plugged in later.
        self.create_timer(0.02, self.read_uwb)
